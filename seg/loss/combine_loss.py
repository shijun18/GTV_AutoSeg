...
class BCEPlusDice(nn.Module):
    ...
    def forward(self, predict, target):
        ...
        assert isinstance(predict,list) or isinstance(predict,tuple)
...
class BCEPlusTopk(nn.Module):
    ...
    """
    def __init__(self, weight=None, ignore_index=None, **kwargs):
        super(BCEPlusTopk, self).__init__()
        self.kwargs = kwargs
        self.weight = weight
        self.ignore_index = ignore_index
    ...
    def forward(self, predict, target):
        ...
        topk_loss = topk(predict[0],target[0])

        bce = nn.BCEWithLogitsLoss(self.weight)
        bce_loss = bce(predict[1],target[1])
        
        total_loss = bce_loss + topk_loss
